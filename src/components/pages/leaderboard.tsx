...
import CatalogueItemRow from '../catalogue/components/catalogue-item-row';
  ...
  return (
    <div className="content">
        ...
        <div className="leaderboard-list">
          {rest.map((item, index) => <div key={index} className="leaderboard-list-item">{`#${4+index}   ${item.address}`}</div>)}
        </div>
		</div>
	</div>
  );
